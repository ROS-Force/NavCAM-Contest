...
class ImageListener:
    ...
    def imageDepthCallback(self, data):
        try:
            ...
            (rows, columns) = cv_image.shape
            pix = (np.int64(columns//2), np.int64(rows//2))
            if self.intrinsics:
                depth = cv_image[pix[1], pix[0]]
                result = rs2.rs2_deproject_pixel_to_point(self.intrinsics, [pix[0], pix[1]], depth)
                print('\rDepth at pixel(%3d, %3d): %4f(mm). Coordinate: %8.2f %8.2f %8.2f\r' % (pix[0], pix[1], cv_image[pix[1], pix[0]], result[0], result[1], result[2]))
            else:
                print('\rDepth at pixel(%3d, %3d): %4f(mm).\r' % (pix[0], pix[1], cv_image[pix[1], pix[0]]))
            #sys.stdout.flush()
